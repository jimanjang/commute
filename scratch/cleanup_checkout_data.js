...
async function main() {
  console.log('--- 1. MySQL: Fetching and cleaning today\'s data from t_secom_workhistory ---');
  ...
  try {
    // A. First clean MySQL
    const [res] = await pool.query("UPDATE t_secom_workhistory SET WCTime = NULL, bWC = 0 WHERE WorkDate = '20260518'");
    console.log(`✅ MySQL: Cleaned up ${res.affectedRows} rows!`);
    
    // B. Query the cleaned rows for BQ load
    const [mysqlRows] = await pool.query("SELECT * FROM t_secom_workhistory WHERE WorkDate = '20260518'");
    rows = mysqlRows;
    console.log(`✅ MySQL: Queried ${rows.length} cleaned rows to upload!`);
  } catch (err) {
  ...
  console.log('\n--- 2. BigQuery: Uploading cleaned records via Load Truncate Job ---');
  try {
