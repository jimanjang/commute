...
async function main() {
  ...
  try {
    ...
    // 2. Fetch MySQL users in target WorkGroups ('002', '006', '007')
    const [mysqlUsers] = await pool.query(
      "SELECT Name, Sabun, Email, WorkGroup FROM t_secom_person WHERE WorkGroup IN ('002', '006', '007')"
    );
    
    console.log(`=== Mismatched Emails for Target WorkGroups (002, 006, 007) | Monitored Count: ${mysqlUsers.length} ===`);
