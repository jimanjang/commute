...
async function main() {
  ...
  try {
    ...
    console.log(`Total roster rows: ${rosterRows.length}`);
    const laikaInRoster = rosterRows.find(r => r.name.includes("라이카") || r.name.includes("Laika"));
    console.log("Laika in Roster:", laikaInRoster);
    ...
    const laikaResult = usersWithStatus.find(u => u.name.includes("라이카") || u.name.includes("Laika"));
    console.log("\n--- Laika Status Calculation Result ---");
    console.log(laikaResult);
