...
function DashboardContent() {
  ...
  const fetchData = async () => {
    ...
    try {
      if (userName === "전체") {
        // ALWAYS fetch users data to show summary stats
        // If date exists, it fetches for that date, otherwise for today
        ...
        {!targetDateParam ? (
          ...
          <div className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
            <div className="p-8 border-b border-gray-100 flex items-center justify-between bg-gray-50/30">
              <div>
                <h2 className="text-[18px] font-extrabold text-slate-800 tracking-tight">{targetDateParam} 기준 출퇴근 현황</h2>
                <p className="text-sm text-gray-400 font-medium mt-1">선택하신 날짜의 근무 상태를 보여줍니다.</p>
              </div>
      ...
      <div className="space-y-8">
        ...
        <div className="sticky top-[94px] space-y-8">
          <DailyDetail 
            ...
            onEditClick={() => {
              // Create a dummy user object for the selected record
              const userObj = {
                name: userName,
                displayName: userName, // Or infer from DB
